...
def format_file_output(file, fields, classification=None):
	...
	if classification and classification.get('label'):
		...
		if reasoning:
			output_parts.append("[{}] {}".format(label, reasoning))
		else:
			output_parts.append("[{}]".format(label))
...
def create_google_sheets_report(user_data, admin_email, ai_enabled=False):
	...
	for user_email, files in user_data.items():
		if files:  # Only create tabs for users with files
			...
			if ai_enabled:
				headers.extend(['AI Classification', 'Reasoning'])
			rows = [headers]
			...
			for file in files:
				...
				if ai_enabled:
					ai = file.get('_ai_classification', {})
					row.extend([ai.get('label', ''), ai.get('reasoning', '')])
				rows.append(row)
	...
	if ai_enabled:
		ai_counts = {}
		for files in user_data.values():
			for file in files:
				label = file.get('_ai_classification', {}).get('label', 'UNKNOWN')
				ai_counts[label] = ai_counts.get(label, 0) + 1
		dashboard_data.append([''])
		dashboard_data.append(['AI Classification Summary:'])
		dashboard_data.append(['Classification', 'Count'])
		for label in ['CRITICAL', 'RESTRICTED', 'CONFIDENTIAL', 'INTERNAL', 'PUBLIC', 'UNKNOWN']:
			if label in ai_counts:
				dashboard_data.append([label, str(ai_counts[label])])
...
if __name__ == "__main__":
	...
	parser.add_argument('--ai', action='store_true',
		help='enable AI-powered data sensitivity classification for each public file')
	parser.add_argument('--ai-prompt', default='', metavar='INSTRUCTIONS',
		help='additional classification instructions appended to the AI prompt at runtime')
	...
	# Load AI classification config if requested
	ai_config = None
	if args.ai:
		try:
			import ai_classifier
			ai_config = ai_classifier.load_ai_config()
			print("AI classification enabled (provider: {})".format(ai_config["provider"]))
			...
			try:
				...
				if drive_files:
					...
					for file in drive_files:
						if ai_config:
							classification = ai_classifier.classify_file(
								file, settings.ADMIN_USERNAME, ai_config, args.ai_prompt)
							file['_ai_classification'] = classification
						print("    {}".format(format_file_output(file, args.fields, file.get('_ai_classification'))))
				else:
		...
		if shared_drive_files:
			...
			if not args.no_html:
				...
				for file in shared_drive_files:
					ai_tag = ""
					if file.get('_ai_classification') and file['_ai_classification'].get('label'):
						ai_tag = " <strong>[{}]</strong>".format(file['_ai_classification']['label'])
					result_elem += "<li><a href=\"{link}\">{name}</a> (Shared Drive: {drive}){ai_tag}</li>\n".format(
		...
		for user in users:
			...
			if public_files:
				...
				for file in public_files:
					if ai_config:
						classification = ai_classifier.classify_file(
							file, user_email, ai_config, args.ai_prompt)
						file['_ai_classification'] = classification
					print("    {}".format(format_file_output(file, args.fields, file.get('_ai_classification'))))
					if not args.no_html:
						ai_tag = ""
						if file.get('_ai_classification') and file['_ai_classification'].get('label'):
							ai_tag = " <strong>[{}]</strong>".format(file['_ai_classification']['label'])
						result_elem += "<li><a href=\"{link}\">{name}</a>{ai_tag}</li>\n".format(
							link=file['webViewLink'], name=file['name'], ai_tag=ai_tag)
	...
	if args.sheets and sheets_data:
		...
		try:
			sheets_url = create_google_sheets_report(sheets_data, settings.ADMIN_USERNAME, ai_enabled=bool(ai_config))
